<template>
    <div id="twPolls" class="my-2">
        <div class="container no-gutters text-decoration-none">
            <div class="row">
                <div v-if="media.length" class="card mx-auto my-3">
                    <div class="row">
                        <el-image :preview-src-list="[createRealMediaPath(realMediaPath, samePath, 'tweets')+media[0].url+'']"
                                  :src="createRealMediaPath(realMediaPath, samePath, 'tweets')+media[0].url+''"
                                  alt="pollImage"
                                  class="col-12 card-img-top" fit="cover" lazy style="height: 300px"></el-image>
                    </div>
                </div>
                <div
                    v-if="(etaSeconds <= 0 && ((!settings.onlineMode && polls[0].checked)) || ((settings.onlineMode && polls.length)) && pollCount > 0)"
                    class="col-12">
                    <div v-for="(poll, index) in polls" :key="poll.poll_order" class="progress">
                        ...
                        <div class="progress-data">
                            <span :class="{'fw-bold': maxPollIndex === index, 'fw-normal': maxPollIndex !== index}">{{
                                    poll.choice_label
                                }}</span>
                            <span :class="{'fw-bold': maxPollIndex === index, 'fw-normal': maxPollIndex !== index}">{{
                ...
                <template v-else>
                    <el-button v-for="poll in polls" :key="poll.poll_order" class="btn-block mx-auto mb-1"
                               round @click="e => {e.stopPropagation()}">{{ poll.choice_label }}
                    </el-button>
                </template>
...
import {Media as MediaInterface, PollItem} from "../types/Content";
...
const props = defineProps({
    ...
    media: {
        type: Array as PropType<MediaInterface[]>,
        default: []
    }
})
